...
- Billiards Table: 经典台球桌，带六个洞口。
- Airbag Square: 正方形场地，四周全部由一次性气囊墙围住。
- Portal Cloud Square: 正方形场地，对边为传送门，内部有云层材质构成的小洞。
- Elastic Pinball: 弹力墙、粘性板、冰面、沙地和气囊混合的弹射场。

官方地图也通过 core 的 `EditableMapDocument` 生成，再转换为 `MapData`，因此与自定义地图共享同一套物理和房间开局流程。
